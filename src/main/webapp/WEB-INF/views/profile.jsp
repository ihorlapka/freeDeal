...
<form:form modelAttribute="user" method="post" action="/updateUser">
    <div>
        <form:label path="profilepicture"><pre><strong>Change your profile picture:
            <br>just leave your link below</strong></pre>
        </form:label>
...
<div>
    <table border="1" width="25%" cellpadding="2" class="friends-profile">
        ...
        <c:forEach items="${user.friends}" var="f">
            <tr>
                ...
                <td>${f.username}
                    <button class="btn-show" onclick="
                            {
                                $('#receiverUsername').text('Messaging with ${f.username}');
                                $('#msgForm').attr('action', '/sendMessage/${f.id}');
                            }">message
                    </button>
    ...
    <div class="message-window" hidden>
        ...
        <br>
        <span id="receiverUsername"></span>
        <form id="msgForm" method="get">
...
<div>
    <table border="1" width="50%" cellpadding="2" class="order-execute">
        ...
        <tr>
            <th>Picture</th>
            <th><a href="/profile?pageExecOrder=${param.get("pageExecOrder")}&sortExecOrder=ordername">Ordername</a>
            </th>
            <th><a href="/profile?pageExecOrder=${param.get("pageExecOrder")}&sortExecOrder=payment">Payment</a></th>
            <th><a href="/profile?pageExecOrder=${param.get("pageExecOrder")}&sortExecOrder=workersamount">Workers</a>
            </th>
